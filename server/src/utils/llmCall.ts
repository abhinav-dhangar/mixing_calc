...
main point : => you will have to give me two output , one is markdown that i have to show , and other one is that text output which will i have to give to llm for making that to speech which will more straight-forward (will not speak to more )
...
export const invokeOutput = async (input:string)=>{
        

      
      const prompt1 = await taggingPrompt.invoke({
        input });
   const response =    await llmWihStructuredOutput.invoke(prompt1);
